...
public class ReminderManager {
    ...
    public void remindAdvisorGradeAnnouncementOneWeek() {
        try {
            ...
            List<QA> unannouncedQAs = qAFacade.findGradedQAs();
            for (QA qa : unannouncedQAs) {
                if (qa.getGradedDate() != null) {
                    long diff = new Date().getTime() - qa.getGradedDate().getTime();
                    long diffDays = diff / (24 * 60 * 60 * 1000);
                    if (diffDays >= 7) {
                        mailNotificationManagerBean.sendAdvisorReminderQAGradeAnnouncementOneWeek(qa);
                    }
                }
            }
        } catch (Exception e) {
    ...
    @Schedule(persistent = false) // TODO CHANGE
